...
export default function Imovel() {
  ...
  useEffect(() => {
    ...
    const getPropertie = useCallback(async () => {
        try{
            const response = await propertiesService.getByUrl(router?.query?.url as string)
          ...
          <div className="flex justify-between w-full 3xl:max-w-screen-3xl xl:max-w-screen-xl md:max-w-screen-lg mx-auto mt-8">
            <div>
              ...
              <p className="text-4xl  text-orange-500 font-bold">
                  {propertie?.price} <span className="text-xl">/ {propertie?.transaction}</span>
              </p>
            </div>
            <div className="gap-2">
          ...
          <div className="flex  justify-between w-full xl:max-w-screen-xl 3xl:max-w-screen-3xl md:max-w-screen-lg mx-auto mt-8">
            <div>
              <div className="grid">
                ...
                <div className="flex flex-wrap gap-4 mt-2">
                  {rooms.map((room, index) => (
                    <span key={index} className="bg-[#FDEAE1] px-2 rounded-lg text-gray-600 3xl:text-md xl:text-sm">
                      {room.title_formated}
                    </span>
                  ))}
                </div>
              ...
              <div className="grid mt-4">
                <p className="font-serif text-lg font-bold text-gray-600">Condomínio</p>
                <div className="flex flex-wrap gap-4 mt-2">
                  {condominioRefactored.map((condominio:string, index:number) => (
                    <span key={index} className="bg-[#FDEAE1] px-2 rounded-lg text-gray-600 3xl:text-md xl:text-sm">
                      {condominio}
                    </span>
                  ))}
              ...
              <div className="grid justify-between w-full xl:max-w-screen-xl 3xl:max-w-screen-3xl md:max-w-screen-lg mx-auto mt-8 border-y py-8" ref={divRef} />
              <div className="grid justify-between w-full xl:max-w-screen-xl 3xl:max-w-screen-3xl md:max-w-screen-lg mx-auto mt-8 w-full">
                <p className="font-serif text-lg font-bold text-gray-600">Localização</p>
                <span className="border p-2 w-full text-gray-600">{adress.formatted}</span>
              </div>
            </div>
